...
export default function Chatbot() {
  ...
  type DetailEntry = {
    [key: string]: number | undefined | string | undefined;
    error?: string;
  };
  ...
  const handleResponse = (data: ResponseData) => {
    ...
    for (const ingredient of ingredients) {
      ...
      if (value && value[totalNutrientKey] !== undefined) {
        ...
        total += value[totalNutrientKey] as number;
    ...
    try {
      ...
      if (data.missing_quantity_for && !data.error) {
        ...
        setMessages((msgs) => [
          ...
          {
            from: "bot",
            text: data.message || "Please provide the quantity.",
          },
        ]);
      } else if (data.error) {
        setMessages((msgs) => [
          ...msgs,
          { from: "bot", text: data.error || "Unknown error." },
        ]);
      } else {
        handleResponse(data);
      }
